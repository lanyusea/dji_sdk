...
#include <iostream>
using namespace std;
static dji_api_msg_t recv_msg_buf={0};
static dji_api_msg_t send_msg_buf={0};
static dji_protocol_state_t g_dji_api_data={0};

API_INTERFACE int16_t init_dji_api_protocol(pfunc_write pf_drc_drite)
{
    //-----------state init
    g_dji_api_data.cur_pack_step        =API_UNPACK_HEADER;
    g_dji_api_data.cur_pack_data_len    =0;
    g_dji_api_data.cur_pack_data_index  =0;
    g_dji_api_data.num_pack_per_second  =0;
    g_dji_api_data.pf_write             =pf_drc_drite;
    return 1;
}
API_INTERFACE int16_t dji_api_send_data(uint16_t dev_id,uint16_t cmd_id,uint8_t* pbuf,uint16_t len)
{
    return dji_api_pack_data2(dev_id,cmd_id,pbuf,len);
}
API_INTERFACE void dji_api_recv_ontick(uint8_t char_data,dev_handler_table_t* p_handler_tab)
{
    dji_api_unpack_by_char(char_data,p_handler_tab);
}
 ...
 int16_t dji_api_unpack_by_char(uint8_t char_data,dev_handler_table_t* p_handler_tab)
{
    ...
    dji_protocol_state_t* pstate;
    pstate = &g_dji_api_data;
